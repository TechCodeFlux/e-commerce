        ...
        <div class="col-md-9">
            ...
            <div class="card mb-4">
                <div class="card-body d-flex align-items-center">
                    ...
                    <div class="ms-auto">
                        <a href="{{ route('club.add_microsites', $club->id) }}" class="btn btn-primary">
...
$(document).ready(function() {
    ...
    table = $('#micrositeTable').DataTable({
        ...
        dom: 'rtip',
        ajax: { 
            url: "{{ route('club.show_microsites', $club->id) }}",
...
// {{ route('admin.microsite_change_status') }}
    $.ajax({
        url: "",
        type: "POST",
        data: {
...
// SHOW MODAL
    document.addEventListener('click', function (e) {
        ...
        if (e.target.closest('.showMicrosite')) {
            ...
            fetch("{{ route('club.microsite_show', ':id') }}".replace(':id', id))
                .then(res => res.json())
                .then(data => {
   ...
   $(document).on('click', '.showMicrositeProducts', function(){
    ...
    let url = "{{ route('club.microsite.list_products', ['microsite' => ':id']) }}";
