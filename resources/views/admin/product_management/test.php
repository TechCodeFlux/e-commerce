...
document.getElementById('imageInput').addEventListener('change', function () {
    ...
    function restoreProductImage() {
        ...
        // }
        // document.getElementById('fileNameDisplay').innerText = data.name;

        // Restore File object to input
        fetch(data.dataUrl)
            .then(res => res.blob())
            .then(blob => {
    ...
    document.getElementById('imageInput').addEventListener('change', function (event) {
        ...
        if (file) {
            ...
            // Show filename
            // let fileLabel = document.getElementById('fileNameDisplay');
            // if (!fileLabel) {
...
// Restore function now only works if you want to keep during the same session
function restoreProductImage() {
    const saved = localStorage.getItem(PRODUCT_IMAGE_KEY);
    if (!saved) return;
...
// ======== Clear image storage on page load ========
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
        localStorage.removeItem(PRODUCT_IMAGE_KEY); // Clear previous saved image
    });
} else {
    localStorage.removeItem(PRODUCT_IMAGE_KEY);
}
